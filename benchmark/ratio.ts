...
describe('Benchmark: Package', async function () {
  ...
  async function run(label: string, source: string, capacity: number) {
    ...
    const tcl = new TClock<number, 1>(capacity);
    ...
    for (let i = 0; i < keys.length; ++i) {
      for (let key = keys[i], cnt = cnts[i]; cnt--; ++key) {
        ...
        stats.tcl += tcl.get(key) ?? (tcl.set(key, 1), 0);
      }
    }
  ...
  function print(label: string, stats: Stats): void {
    ...
    console.log('TClock hit ratio', `${format(stats.tcl * 100 / stats.total, 2)}%`);
    console.log('TClock - LRU   hit ratio delta', `${format((stats.tcl - stats.lru) * 100 / stats.total, 2)}%`);
    console.log('TClock - Clock hit ratio delta', `${format((stats.tcl - stats.clk) * 100 / stats.total, 2)}%`);
    console.log('');
  }
